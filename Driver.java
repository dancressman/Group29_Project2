	...
	public static Database searchHub(Scanner scnr, Database database, 
		...
		if (parameterSwitch == 1 || parameterSwitch == 3) {
			...
			if (mediaSwitch == 1 || mediaSwitch == 3) {
				ArrayList<Movie> matches;
				
				// Uses exact method if exact match is required
				if (precisionSwitch == 1)
					matches = database.searchMovieTitleExact(titleSearch);
				else
					matches = database.searchMovieTitle(titleSearch);
			 ...
			 */
			if (mediaSwitch == 2 || mediaSwitch == 3) {
				ArrayList<Series> matches;
				
				// Uses exact method if exact match is required
				if (precisionSwitch == 1)
					matches = database.searchSeriesTitleExact(titleSearch, 
						includeEpisodes);
				else
					matches = database.searchSeriesTitle(titleSearch, 
							includeEpisodes);
		...
		if (saveInput.equalsIgnoreCase("Y") || saveInput.equalsIgnoreCase("YES")) {
			...
			System.out.println("Search results saved.");
		}
